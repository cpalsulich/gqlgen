...
import (
	_ "github.com/cpalsulich/gqlgen"
)
```
...
```shell
go get -d github.com/cpalsulich/gqlgen@VERSION
```
...
And run `go run github.com/cpalsulich/gqlgen generate`.

>
> If you run into this error `package github.com/cpalsulich/gqlgen: no Go files` while executing the `generate` command above, follow the instructions in [this](https://github.com/cpalsulich/gqlgen/issues/800#issuecomment-888908950) comment for a possible solution.
...
```go
//go:generate go run github.com/cpalsulich/gqlgen generate
```
